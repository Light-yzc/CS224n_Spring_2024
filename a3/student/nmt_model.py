...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.encoder = nn.LSTM(embed_size, hidden_size,bidirectional=True) # default have bias
        self.decoder = nn.LSTMCell(embed_size+hidden_size, hidden_size) #I miss this,my origin code is //self.decoder = nn.LSTMCell(embed_size, hidden_size)
        self.h_projection = nn.Linear(hidden_size*2, hidden_size, bias=False)
        self.c_projection = nn.Linear(hidden_size*2, hidden_size, bias=False)
        self.att_projection = nn.Linear(hidden_size*2, hidden_size, bias=False)
        self.combined_output_projection = nn.Linear(hidden_size*3, hidden_size, bias = False)  
        self.target_vocab_projection = nn.Linear(hidden_size,len(vocab.tgt), bias=False)  #wrong code before // self.target_vocab_projection = nn.Linear(vocab,hidden_size, bias=False)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        time_Y = torch.split(Y, 1)
        for i in range(Y.shape[0]):
            Y_t = torch.squeeze(time_Y[i])
            Ybar_t = torch.cat([Y_t, o_prev], dim = 1)
            dec_state, o_t, e_t = self.step(Ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks)
            combined_outputs.append(o_t)
            o_prev = o_t
        ### END YOUR CODE
        combined_outputs = torch.stack(combined_outputs)#wrong //combined_outputs = torch.stack((target_padded.shape[0], combined_outputs))
