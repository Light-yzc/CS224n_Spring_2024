...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        elif transition == 'LA': self.dependencies.append((self.stack[-1], self.stack.pop(-2)))
        elif transition == 'RA': self.dependencies.append((self.stack[-2], self.stack.pop()))
        else: ValueError('Undefined Code')
...
def minibatch_parse(sentences, model, batch_size):
    ...
    def is_End(pp):
        return True if len(pp.buffer) == 0 and len(pp.stack) == 1  else False
    partial_parses = [PartialParse(x) for x in sentences]
    unfinished_parses = partial_parses[:]
    while len(unfinished_parses) > 0:
        min_batch = unfinished_parses[:batch_size]
        next_step = model.predict(min_batch)

        for pp, transition in zip(min_batch,next_step):
